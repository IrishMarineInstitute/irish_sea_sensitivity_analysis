# ---------------------------------------------------
# Minimum Cost reserves. SACs excluded. All features, Combined costs
...
setwd("../SAC")
SAC_1km <- raster("SAC_NPWS_PA.tif")
SAC_3km <-aggregate(SAC_1km, fact=3) 
spplot(SAC_3km)
...
for (i in 1:nlayers(feat_stack_1km)) {
  ...
  feat_layer <- feat_stack_1km[[i]]
  feat_layer[is.na(feat_layer) & !is.na(pu_1km)] <- 0
  feat_stack_1km[[i]] <- feat_layer
}

spplot(feat_stack_1km)

# Aggreate the feature stack to 3 km scale
feat_stack_3km <-aggregate(feat_stack_1km, fact=3) # aggregate feat_stack_1km in most cases
spplot(feat_stack_3km)

# Remove extent of SACs from layers
for (i in 1:nlayers(feat_stack_3km)) {
  # Extract the i-th layer from the stack
  feat_layer <- feat_stack_3km[[i]]
  feat_layer[SAC_3km == 1] <- NA
  feat_stack_3km[[i]] <- feat_layer
}
...
# Remove extent of SACs from layers
for (i in 1:nlayers(cost_stack_3km)) {
  # Extract the i-th layer from the stack
  cost_layer <- cost_stack_3km[[i]]
  cost_layer[SAC_3km == 1] <- NA
  cost_stack_3km[[i]] <- cost_layer
}
spplot(cost_stack_3km)
...
scenario_name <- "NoSAC_LA_AllCost_AllFeat_1_corrected_V2_PB" #Each scenario needs a unique name
scenario_group <- "SAC Locked Out"
...
targets <- (rep(0.19, length(feature_names)))
targets[15] <- 0.51 #sets Fronts to higher target
...
penalty <- round(seq(prelim_penalty[2], prelim_penalty[4], length.out = 9), 2)
penalty
...
solution <- lapply(penalty, function(x) {
  s <-
    ...
    add_boundary_penalties(penalty = x, data = bld_3km) %>%
    add_neighbor_constraints(k = 2) %>%
    add_gurobi_solver(threads = parallel::detectCores(TRUE) - 1, gap = 0.03, time_limit = 1200, verbose = TRUE) %>%
    solve()
  s <- stack(s)
  names(s) <- paste0("bp_", x)
  return(s)
...
best_solution <- solution_layers[[7]] #NB different from the previous run
...
# Note value of 0.5 used to give different colour to SAC on plot
best_solution_SAC <- best_solution
best_solution_SAC[SAC_3km == 1] <- 0.5

spplot(best_solution_SAC)

png(paste0(scenario_name, "_solutionPlusSAC.png"), width = 1200, height = 1200, res = 300)
spplot(best_solution_SAC, axes = FALSE, box = FALSE)
dev.off()

writeRaster(best_solution_SAC, paste0(scenario_name, "_bestsolutionPlusSAC.tif"), format = "GTiff")
