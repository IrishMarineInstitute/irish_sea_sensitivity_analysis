# ---------------------------------------------------
# Minimum Cost reserves. All features, Combined costs
...
setwd("mpa_ecological_sensitivity/Test_Scenarios") #File containing the features
...
setwd("../data/PreppedCostLayers") #Folder with the cost layers
...
setwd("../planning_units") #Folder with the planning units file
...
scenario_name <- "MCR_AFCC_PB" #Each scenario needs a unique name
...
targets <- (rep(0.28, length(feature_names))) #targets 0.2 not meeting 30% target. 
#targets[1] <- 0.4 #NOTE THIS CURRENTLY CHANGES THIS EEL SETTINGS. Eel treated the same as other species
targets[15] <- 0.6 #target of 0.8 set for the fronts
...
tracking <- read.csv(file = "Scenarios.csv")    # TRISH CHANGE NAME TO "Scenarios_PB.csv"
